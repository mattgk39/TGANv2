...
def genSamples(g, n=8):
    ...
    try:
        ...
        clip.write_gif('2_lossgp_generated_video.gif', fps=8)
...
class Generator(Model):
    ...
    def sub_sample(self, x, frame=2):
        original_shape = x.shape  # Logging original shape
        offset = 0  # If you want to use random offset, you need to ensure it fits within the frame count.
    ...
    def call(self, z):
        ...
        if self.training:
            ...
            gen_train_output = (x1, x2, x3, x4)
            print(f"Generator training call finished, output shapes = {x1.shape}, {x2.shape}, {x3.shape}, {x4.shape}")
            return x1, x2, x3, x4
...
def log_tensor(x, name):
    ...
    min_val = tf.math.reduce_min(x_flat)
    max_val = tf.math.reduce_max(x_flat)
    mean_val = tf.math.reduce_mean(x_flat)
    print(f"{name} - Shape:{x.shape}, Min: {min_val.numpy()}, Max: {max_val.numpy()}, Mean: {mean_val.numpy()}")
...
class DisBlock(Model):
    ...
    def down_sample(self, x):
        ...
        pad = [(0 if k % 2 == 0 else 1) for k in x.shape[1:4]]  # For depth, height, width
        ...
        ksize = [(2 if k > 1 else 1) for k in x.shape[1:4]]
...
class ResNet3D(Model):
    ...
    def extract_features(self, x):
        ...
        x = tf.reduce_sum(x, axis=(1, 2, 3)) #Correct?
        return x
    
    def call(self, x):
        #print("Input to ResNet3D - Shape:", x.shape)
        #log_tensor(x, 'ResNet Input')
        x = self.extract_features(x)
        #log_tensor(x, 'Extract_Features Output')
        x = self.fc(x)
        #print("Output from ResNet3D - Shape:", x.shape)
        return x
...
class Discriminator(Model):
    ...
    def call(self, xs):
        ...
        outputs = []
        i=1
        for level, x in zip(self.levels, xs):
            #print(f"Level {i}")
            #print("Processing level with input shape:", x.shape)
            output = level(x)
            outputs.append(output)
            #print("Output shape from level:", output.shape)
            #print("\n")
            i += 1
        #Stack ouputs
...
def compute_gradient_penalty(inputs, disc_outputs, lam, tape):
    gradient_penalty_sum = 0.0  # Initialize the sum of gradient penalties
    #print(f"inputs type is {type(inputs)}")
    #print(f"disc_outputs type is {type(disc_outputs)}")
    ...
    for input_tensor in inputs:
        ...
        #print(f"Gradients type is {type(gradients)}")
        #print("Got gradients")
...
def train(train_values, gen_values, disc_values, gen_lr=0.0001, disc_lr=0.0001, save_interval=50):
    ...
    #Extracct values from disc_values and instantiate the discriminator
    ...
    for epoch in range(n_epochs):
        ...
        for batch in dataloader:
            print(f"Beginning batch iteration {iteration}")
            ...
            # Discriminator real data prediction
            ...
            with tf.GradientTape(persistent=True) as gen_tape, tf.GradientTape() as disc_tape:
                ...
                #print("Computing loss_gp")
                #loss_gp = compute_gradient_penalty(xd, yd, lambda_gp, gp_tape)
                #print(f"loss_gp type: {type(loss_gp)}, loss_gp value: {loss_gp}")
            ...
            try:
                ...
                print("Floating point exception detected:", e)
            ...
            print("Applied gen gradient")

            #Compute and apply gradient penalty gradients
            #print("Computing loss_gp gradients")
            #gp_gradients = gp_tape.gradient(loss_gp, disc.trainable_variables)

            #Apply gradient penalty
            #print("Applying loss_gp gradients")
            #gradients = gp_tape.gradient(loss_gp, disc.trainable_variables)
            #disc_opt.apply_gradients(zip(gp_gradients, disc.trainable_variables))
        ...
        print(f"Epoch {epoch+1}/{n_epochs} complete. Average Losses: gen={avg_gen_loss}, disc={avg_disc_loss}, real={avg_loss_real}")
